...
export default function AllCategorySection() {
  ...
  // Particle জেনারেট (একবারই হবে)
  const particles = useMemo<Shape[]>(() => {
  ...
  // Particle রেন্ডার
  const renderShape = (p: Shape, i: number) => {
    ...
    switch (dynamicShape) {
      case "circle":
        return (
          <motion.div key={i} drag dragConstraints={{ top: 0, left: 0, right: windowSize.width, bottom: windowSize.height }}
            style={{ width: p.size, height: p.size, borderRadius: "50%", backgroundColor: dynamicColor, ...commonStyle }}
            animate={animationProps} transition={transitionProps} />
        );
      case "star":
        return (
          <motion.div key={i} drag dragConstraints={{ top: 0, left: 0, right: windowSize.width, bottom: windowSize.height }}
            style={{ width: 0, height: 0, borderLeft: `${p.size}px solid transparent`, borderRight: `${p.size}px solid transparent`, borderBottom: `${p.size * 2}px solid ${dynamicColor}`, ...commonStyle }}
            animate={animationProps} transition={transitionProps} />
        );
      case "triangle":
        return (
          <motion.div key={i} drag dragConstraints={{ top: 0, left: 0, right: windowSize.width, bottom: windowSize.height }}
            style={{ width: 0, height: 0, borderLeft: `${p.size}px solid transparent`, borderRight: `${p.size}px solid transparent`, borderTop: `${p.size * 2}px solid ${dynamicColor}`, ...commonStyle }}
            animate={animationProps} transition={transitionProps} />
        );
      default:
        return null;
  ...
  return (
    <div onClick={handleClick} onTouchStart={handleClick} className="relative min-h-screen flex flex-col items-center justify-center p-6 bg-black overflow-hidden">
      {/* Background Effects */}
      <div className="absolute inset-0 grid bg-transparent before:content-[''] before:absolute before:inset-0 before:bg-[repeating-linear-gradient(0deg,#0ff0 0 1px,#0000 1px_20px)] after:content-[''] after:absolute after:inset-0 after:bg-[repeating-linear-gradient(90deg,#0ff0 0 1px,#0000 1px_20px)] animate-[pulse_10s_linear_infinite] pointer-events-none"></div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 w-full max-w-7xl z-10 relative">
        {categories.map((cat, index) => (
          <motion.div key={index} whileHover={{ scale: 1.08, rotate: 2 }} whileTap={{ scale: 0.95 }}
            ...
            <div className={`absolute inset-0 bg-gradient-to-br ${cat.color} opacity-60 group-hover:opacity-80 transition-all duration-500`} />
            ...
            <AnimatePresence>
              {clickBursts.map((burst) => (
                <motion.div key={burst.id} initial={{ x: burst.x, y: burst.y, opacity: 1, scale: 1 }}
                  animate={{ x: burst.x + (Math.random() - 0.5) * 100, y: burst.y + (Math.random() - 0.5) * 100, opacity: 0, scale: 0 }}
                  exit={{ opacity: 0 }} transition={{ duration: 1 }}>
                  <div style={{ width: burst.size, height: burst.size, backgroundColor: burst.color, borderRadius: "50%", filter: `drop-shadow(0 0 6px ${burst.color})` }} />
                </motion.div>
              ))}
            </AnimatePresence>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-white z-10 p-4 space-y-3">
              ...
              <Link href={cat.link}>
                <motion.button whileHover={{ scale: 1.1, textShadow: "0 0 15px #fff", boxShadow: "0 0 20px #fff" }}
